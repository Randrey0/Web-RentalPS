...
class ModelBooking extends CI_Model
{
     ...
     return $this->db->get($table);
      ...
      public function hapusSewa($where = null)
    {
     ...
     public function joinOrder($where)
    {
     ...
     $this->db->join('console bu ', 'bu.id=d.id_console');
     $this->db->where($where);
     return $this->db->get();
     }
     public function simpanDetail($where = null)
     {
     $sql = "INSERT INTO booking_detail (id_booking,id_console) SELECT booking.id_booking,temp.id_console FROM booking, temp WHERE temp.id_user=booking.id_user AND booking.id_user='$where'";
     $this->db->query($sql);
     }
     ...
     $this->db->limit(1);
     return $this->db->get('console', $where);
     }
    ...
    public function createTemp()
     {
     $this->db->query('CREATE TABLE IF NOT EXISTS temp(id_booking varchar(12), tgl_booking DATETIME, email_user varchar(128), id_console int)');
     }
     ...
     $this->db->join('console', 'booking_detail.id_console=console.id');
     return $this->db->get();
     }
