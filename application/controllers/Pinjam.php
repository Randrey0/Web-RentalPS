...
class Pinjam extends CI_Controller
{
    ...
    public function bookingDetail()
    {
        ...
        $data['detail'] = $this->db->query("select id_console,nama_console, harga_sewa from booking_detail d, console b where d.id_console=b.id and d.id_booking='$id_booking'")->result_array();
        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
    ...
    public function pinjamAct()
    {
        ...
        //hapus Data booking yang consolenya diambil untuk dipinjam
        ...
        //update dibooking dan dipinjam pada tabel console saat console yang dibooking diambil untuk dipinjam
        $this->db->query("UPDATE console, detail_pinjam SET console.dipinjam=console.dipinjam+1, console.dibooking=console.dibooking-1 WHERE console.id=detail_pinjam.id_console");
    ...
    public function ubahStatus()
    {
        ...
        $where = ['id_console' => $this->uri->segment(3),];
        $tgl = date('Y-m-d');
        $status = 'Kembali';
        //update status menjadi kembali pada saat console dikembalikan
        $this->db->query("UPDATE pinjam, detail_pinjam SET pinjam.status='$status', pinjam.tgl_pengembalian='$tgl' WHERE detail_pinjam.id_console='$id_console' AND pinjam.no_pinjam='$no_pinjam'");
        //update stok dan dipinjam pada tabel console
        $this->db->query("UPDATE console, detail_pinjam SET console.dipinjam=console.dipinjam-1, console.stok=console.stok+1 WHERE console.id=detail_pinjam.id_console");
